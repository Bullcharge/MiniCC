...
public class SemanticChecker {
	...
	public void buildSymbolTable(TreeNode root, SymbolTable st) {
		...
		else if(root.getType() == TreeNodeType.TNT_STMT_DCL) {
			SymbolEntry se = new SymbolEntry();
			TreeNode id = root.getChildByIndex(1);
			int tknIndex = id.getTknIndex();
			String name = (String) Scanner.getTknValueList().get(tknIndex);
	...
	public void printSymbolTable(TreeNode root) {
		if(root.getType() == TreeNodeType.TNT_CMPL_UNIT || root.getType() == TreeNodeType.TNT_FUNC_DEF)
		{
			...
			if(t != null) {
				...
				for(SymbolEntry se: t.getSymbolList()) {
					if(symHash.keySet().contains(se.name)) {
						System.out.println("Symbol " + se.name + " is redefiedl");
					}else {
	...
	/*	
		public void checkUnInit(TreeNode root, SymbolTable st) {
			...
			else if(root.getType() == TreeNodeType.TNT_STMT_ASSIGN) {
			...
			int index = 0;
			for(TreeNode n: root.getChildren()) {
				if(root.getType() == TreeNodeType.TNT_STMT_DCL) {
					return;
				}else if(root.getType() == TreeNodeType.TNT_STMT_ASSIGN) {
					if(index == 0) {
						continue;
	...
	public void parsing2AST(TreeNode root) {
		if(root.getType() == TreeNodeType.TNT_CMPL_UNIT)
		{
			if(root.getChildren().size() == 1) {
				...
				TreeNode funcDef = root.getChildByIndex(0);
			...
			if(root.getChildren().size() == 1) {
				TreeNode stmtList = root.getChildByIndex(0);
				TreeNode stmt = root.getChildByIndex(0);
				
				root.getChildren().add(stmt);
				
				while(stmtList.getChildren().size() >= 2) {
					stmtList = stmtList.getChildByIndex(1);
					
					stmt = stmtList.getChildByIndex(0);
					root.getChildren().add(stmt);
				}
				root.getChildren().remove(0); 
		     }
